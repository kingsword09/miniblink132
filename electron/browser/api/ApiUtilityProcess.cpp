...
public:
    ...
    explicit ApiUtilityProcess(v8::Isolate* isolate, v8::Local<v8::Object> wrapper, const std::string& modulePath);
    ~ApiUtilityProcess();
    void close();
...
void ApiUtilityProcess::newFunction(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    new ApiUtilityProcess(isolate, info.This(), modulePath); // v8管理内存回收
...
bool ApiUtilityProcess::Accept(mojo::Message* mojoMessage)
{
    ...
    //
    //     v8::Local<v8::Value> msvV8 = deserializeV8Value(v8::Isolate::GetCurrent(), message.encoded_message);
    //     mate::EventEmitter<ApiUtilityProcess>::emit("message", msvV8);
...
NODE_MODULE_CONTEXT_AWARE_BUILTIN_SCRIPT_MANUAL(atom_browser_utility_process, initializeApiUtilityProcess, &nativeUtilityProcessNative)
