...
public:
    ...
    static WideString UTF8ToUTF16(const std::string& utf8)
    {
        return MultiByteToUTF16(CP_UTF8, utf8);
    }

    static WideString MultiByteToUTF16(int codepage, const std::string& utf8)
    {
        WideString utf16;
        ...
        utf16.resize(n);
        utf16.assign(reinterpret_cast<const WideString::value_type*>(&wbuf[0]), n);
        return utf16;
    }

    static std::string UTF16ToUTF8(const WideString& utf16)
    {
        std::string utf8;
        size_t n = ::WideCharToMultiByte(CP_UTF8, 0, reinterpret_cast<LPCWSTR>(utf16.c_str()), utf16.size(), NULL, 0, NULL, NULL);
        if (0 == n)
            return std::string();
        std::vector<char> buf(n + 1);
        ::WideCharToMultiByte(CP_UTF8, 0, reinterpret_cast<LPCWSTR>(utf16.c_str()), utf16.size(), &buf[0], n, NULL, NULL);
        utf8.resize(n);
        utf8.assign(&buf[0], n);
        return utf8;
    }
