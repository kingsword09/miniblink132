...
DemoHost::DemoHost(gfx::AcceleratedWidget widget, const gfx::Size& size, mojo::PendingReceiver<viz::mojom::FrameSinkManagerClient> client_receiver,
    mojo::PendingRemote<viz::mojom::FrameSinkManager> frame_sink_manager_remote, scoped_refptr<base::SingleThreadTaskRunner> service_runner)
    : widget_(widget)
    , size_(size)
    , thread_("DemoHost")
    , service_runner_(service_runner)
{
    ...
    thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&DemoHost::Initialize, base::Unretained(this), std::move(client_receiver), std::move(frame_sink_manager_remote)));

    host_frame_sink_manager_.SetServiceRunner(service_runner);
}
...
{
    thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&DemoHost::ResizeOnThread, base::Unretained(this), size));
}
...
void DemoHost::ResizeOnThread(const gfx::Size& size)
{
    ...
    service_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            [](base::WeakPtr<DemoHost> self, const gfx::Size& size) { self->display_private_->Resize(size); }, weak_ptr_factory_.GetWeakPtr(), size));
...
// 这个好像就是嵌入那个三角形
void DemoHost::EmbedClients(DemoClient* embedder_client, const gfx::Rect& child_bounds)
{
#if 1
    ...
    host_frame_sink_manager_.RegisterFrameSinkId(frame_sink_id, this, viz::ReportFirstSurfaceActivation::kNo);
    host_frame_sink_manager_.RegisterFrameSinkHierarchy(embedder_client->frame_sink_id(), frame_sink_id);
    ...
    host_frame_sink_manager_.CreateCompositorFrameSink(frame_sink_id, std::move(sink_receiver), std::move(client_remote));
    ...
    viz::LocalSurfaceId lsid_allocation = embedder_client->Embed(frame_sink_id, child_bounds);
    std::unique_ptr<DemoClient> embedded_client = std::make_unique<DemoClient>(frame_sink_id, lsid_allocation, child_bounds, service_runner_);
    embedded_client->Initialize(std::move(client_receiver), std::move(sink_remote));
    ...
    if (embedder_client == root_client_.get()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&DemoHost::EmbedClients, base::Unretained(this), embedded_client.get(), gfx::Rect(125, 125, 150, 150)), base::Seconds(1));
    }
    embedded_clients_.push_back(std::move(embedded_client));
#endif
}

void DemoHost::Initialize(mojo::PendingReceiver<viz::mojom::FrameSinkManagerClient> receiver, mojo::PendingRemote<viz::mojom::FrameSinkManager> remote)
{
    ...
    ::mojo::PendingReceiver<::viz::mojom::CompositorFrameSinkClient> client_receiver
        = root_params->compositor_frame_sink_client.InitWithNewPipeAndPassReceiver();
    root_params->display_private = display_private_.BindNewEndpointAndPassReceiver();
    ...
    allocator_.GenerateId();
    root_client_ = std::make_unique<DemoClient>(root_frame_sink_id, allocator_.GetCurrentLocalSurfaceId(), gfx::Rect(size_), service_runner_);

    host_frame_sink_manager_.BindAndSetManager(std::move(receiver), nullptr, std::move(remote));
    host_frame_sink_manager_.RegisterFrameSinkId(root_params->frame_sink_id, this, viz::ReportFirstSurfaceActivation::kNo);
    host_frame_sink_manager_.CreateRootCompositorFrameSink(std::move(root_params));

    service_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            [](base::WeakPtr<DemoHost> self, const gfx::Size& size) {
                self->display_private_->Resize(size);
                self->display_private_->SetDisplayVisible(true);
            },
            weak_ptr_factory_.GetWeakPtr(), size_));
...
#if 1
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&DemoHost::EmbedClients, base::Unretained(this), root_client_.get(), gfx::Rect(50, 50, 300, 300)), base::Seconds(1));
#endif
}
...
void DemoHost::OnFrameTokenChanged(uint32_t frame_token, base::TimeTicks activation_time)
{
}
