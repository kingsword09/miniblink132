...
DemoClient* g_demo_client = nullptr;

DemoClient::DemoClient(const viz::FrameSinkId& frame_sink_id, const viz::LocalSurfaceId& local_surface_id, const gfx::Rect& bounds,
    ...
    , service_runner_(service_runner)
{
    ...
    sprintf_s(output, 0x99, "DemoClient: %p\n", this);
    ...
    mojo::PendingReceiver<viz::mojom::CompositorFrameSinkClient> receiver, mojo::PendingAssociatedRemote<viz::mojom::CompositorFrameSink> sink_remote)
{
    thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&DemoClient::InitializeOnThread, base::Unretained(this), std::move(receiver), std::move(sink_remote), mojo::NullRemote()));
}
    ...
    mojo::PendingReceiver<viz::mojom::CompositorFrameSinkClient> receiver, mojo::PendingRemote<viz::mojom::CompositorFrameSink> sink_remote)
{
    thread_.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&DemoClient::InitializeOnThread, base::Unretained(this), std::move(receiver), mojo::NullAssociatedRemote(), std::move(sink_remote)));
}

viz::LocalSurfaceId DemoClient::Embed(const viz::FrameSinkId& frame_sink_id, const gfx::Rect& bounds)
{
...
void DemoClient::Resize(const gfx::Size& size, const viz::LocalSurfaceId& local_surface_id)
{
...
// 这个函数被我改成是在vizCompositorThread创建的
viz::CompositorFrame DemoClient::CreateFrame(const viz::BeginFrameArgs& args)
{
    ...
    render_pass->SetNew(kRenderPassId, output_rect, damage_rect, gfx::Transform());
    ...
    for (auto& iter : embeds_) {
        ...
        const gfx::Vector2dF center(child_bounds.width() / 2, child_bounds.height() / 2);
        ...
        viz::SurfaceDrawQuad* embed = render_pass->CreateAndAppendDrawQuad<viz::SurfaceDrawQuad>();
        ...
        embed->SetNew(quad_state,
            ...
            /*visible_rect=*/gfx::Rect(child_bounds.size()), viz::SurfaceRange(surface_id), SkColors::kGray,
    ...
    viz::SolidColorDrawQuad* color_quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
    color_quad->SetNew(quad_state, output_rect, output_rect, colors[(++frame_count_ / 60) % std::size(colors)], false);
...
viz::mojom::CompositorFrameSink* DemoClient::GetPtrImpl()
{
...
viz::mojom::CompositorFrameSink* DemoClient::GetPtr()
{
    ...
    for (; !ret; ret = GetPtrImpl()) {
        base::PlatformThread::Sleep(base::Milliseconds(10));
    }
    return ret;
}

void DemoClient::InitializeOnThread(mojo::PendingReceiver<viz::mojom::CompositorFrameSinkClient> receiver,
    mojo::PendingAssociatedRemote<viz::mojom::CompositorFrameSink> associated_sink_remote, mojo::PendingRemote<viz::mojom::CompositorFrameSink> sink_remote)
{
    ...
    service_runner_->PostTask(
        FROM_HERE, base::BindOnce([](base::WeakPtr<DemoClient> self) { self->GetPtr()->SetNeedsBeginFrame(true); }, weak_ptr_factory_.GetWeakPtr()));
}

void DemoClient::DidReceiveCompositorFrameAck(std::vector<viz::ReturnedResource> resources)
{
    // See documentation in mojom for how this can be used.
}

void DemoClient::OnBeginFrame(const viz::BeginFrameArgs& args, const viz::FrameTimingDetailsMap& timing_details)
{
    ...
    base::AutoLock lock(lock_);
    //GetPtr()->SubmitCompositorFrame(local_surface_id_, CreateFrame(args), absl::optional<viz::HitTestRegionList>(), /*trace_time=*/0);
    viz::CompositorFrame frame = CreateFrame(args);

    service_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            [](base::WeakPtr<DemoClient> self, viz::CompositorFrame frame) {
                self->GetPtr()->SubmitCompositorFrame(self->local_surface_id_, std::move(frame), absl::optional<viz::HitTestRegionList>(), /*trace_time=*/0);
            },
...
void DemoClient::ReclaimResources(std::vector<viz::ReturnedResource> resources)
{
}
