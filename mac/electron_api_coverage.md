...
| powerSaveBlocker | `start(type)`, `stop(id)`, `isStarted(id)` plus Win32 execution-state blocker semantics | Stateful JS shim and Win32-compatible `SetThreadExecutionState` state transition shim | Partial, `power-save-blocker-*` covers execution-state behavior | Add native macOS IOPM assertions and JS runtime smoke for blocker IDs/types |
