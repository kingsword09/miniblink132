...
class PerfCounterTrackTable : public macros_internal::MacroTable {
 public:
  ...
  struct ColumnIndex {
    ...
    static constexpr uint32_t perf_session_id = 10;
    static constexpr uint32_t cpu = 11;
    static constexpr uint32_t is_timebase = 12;
  };
  struct ColumnType {
  ...
  class ConstRowReference : public macros_internal::AbstractConstRowReference<
   ...
   public:
    ...
    ColumnType::is_timebase::type is_timebase() const {
      return table()->is_timebase()[row_number_];
    }
  };
  static_assert(std::is_trivially_destructible_v<ConstRowReference>,
                "Inheritance used without trivial destruction");
  class RowReference : public ConstRowReference {
   ...
   private:
    PerfCounterTrackTable* mutable_table() const {
      return const_cast<PerfCounterTrackTable*>(table());
    }
  };
  static_assert(std::is_trivially_destructible_v<RowReference>,
                "Inheritance used without trivial destruction");
  ...
  class ConstIterator : public macros_internal::AbstractConstIterator<
   ...
   public:
    ...
    ColumnType::is_timebase::type is_timebase() const {
      const auto& col = table()->is_timebase();
      return col.GetAtIdx(
        iterator_.StorageIndexForColumn(col.index_in_table()));
    }
  ...
  class Iterator : public ConstIterator {
    public:
     RowReference row_reference() const {
       return {const_cast<PerfCounterTrackTable*>(table()), CurrentRowNumber()};
     }
...
,
    ...
    static_assert(
      ...
      static_assert(
        ...
        "Column type and flag combination is not valid");
    OnConstructionCompletedRegularConstructor(
      {const_parent_->storage_layers()[ColumnIndex::id],const_parent_->storage_layers()[ColumnIndex::type],const_parent_->storage_layers()[ColumnIndex::name],const_parent_->storage_layers()[ColumnIndex::parent_id],const_parent_->storage_layers()[ColumnIndex::source_arg_set_id],const_parent_->storage_layers()[ColumnIndex::machine_id],const_parent_->storage_layers()[ColumnIndex::classification],const_parent_->storage_layers()[ColumnIndex::dimensions],const_parent_->storage_layers()[ColumnIndex::unit],const_parent_->storage_layers()[ColumnIndex::description],perf_session_id_storage_layer_,cpu_storage_layer_,is_timebase_storage_layer_},
  ...
  static std::unique_ptr<PerfCounterTrackTable> ExtendParent(
...
, ColumnStorage<ColumnType::is_timebase::stored_type> is_timebase) {
    return std::unique_ptr<PerfCounterTrackTable>(new PerfCounterTrackTable(
        parent.string_pool(), parent, RowMap(0, parent.row_count()),
        std::move(perf_session_id), std::move(cpu), std::move(is_timebase)));
  }

  static std::unique_ptr<PerfCounterTrackTable> SelectAndExtendParent(
      const CounterTrackTable& parent,
...
,
    ...
    OnConstructionCompleted(
      {const_parent_->storage_layers()[ColumnIndex::id],const_parent_->storage_layers()[ColumnIndex::type],const_parent_->storage_layers()[ColumnIndex::name],const_parent_->storage_layers()[ColumnIndex::parent_id],const_parent_->storage_layers()[ColumnIndex::source_arg_set_id],const_parent_->storage_layers()[ColumnIndex::machine_id],const_parent_->storage_layers()[ColumnIndex::classification],const_parent_->storage_layers()[ColumnIndex::dimensions],const_parent_->storage_layers()[ColumnIndex::unit],const_parent_->storage_layers()[ColumnIndex::description],perf_session_id_storage_layer_,cpu_storage_layer_,is_timebase_storage_layer_}, {{},{},{},const_parent_->null_layers()[ColumnIndex::parent_id],const_parent_->null_layers()[ColumnIndex::source_arg_set_id],const_parent_->null_layers()[ColumnIndex::machine_id],{},const_parent_->null_layers()[ColumnIndex::dimensions],{},{},{},{},{}}, std::move(overlay_layers));
  }
